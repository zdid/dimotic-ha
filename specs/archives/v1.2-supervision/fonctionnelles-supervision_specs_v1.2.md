...
- Le **core** ne fournit qu'un **emplacement** sur la page d'accueil. Si l'application `supervision`
  est active, il y charge **l'affichage fourni par l'application** (fragment HTML + script de
  `applications/supervision/presentation/`, même mécanisme de chargement que les pages
  d'applications) ; sinon il garde le bloc actuel « Applications sur les autres machines ».
...
| Menu | aucun — affichage sur la page d'accueil (§6) |
...
`enabledByDefault: true` dans sa déclaration (`ApplicationModule`) ; le core la **active** à sa
première apparition au lieu de la désactiver. SUPERVISION est la seule à le déclarer. L'utilisateur
...
1. Côté sauvegarde : marqueurs par cadence + cadences en échec dans le script, échange `sauvegarde:supervision:status` (§5).
2. Côté core : `enabledByDefault` (§7.1) et emplacement de la page d'accueil (§6.2).
3. Application `supervision` : inventaire par gossip (§3), présence/état (§3bis), sélection diffusée + copie locale (§4).
4. Affichage sur la page d'accueil (§6) et colonne sauvegardes (§5).
