...
`AppService` scanne `applications/` à la racine — chaque app a son propre `dist/`/`src/`, pas de
`dist/applications/` partagé. Conditions de détection : `domain/index.ts` exporte
`{NOM_APP}_APP: ApplicationModule` + une factory de service.

Activation/désactivation : liste `disabledApps` dans `data/core/config.yaml`
(`ApplicationManager.ts`) — **le seul interrupteur réel**. Le champ `enabled` d'un schéma de config
d'app individuel n'est lu nulle part pour ça (piège découvert le 06/09/2026).

Si le démarrage échoue (mauvaise config, matériel absent) : warning loggé, pas d'erreur bloquante,
l'app continue de tourner. Redémarrage automatique du service sur
`app:module:config:saved` (config sauvegardée) ou réactivation.
