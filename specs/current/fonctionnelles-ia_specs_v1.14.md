...
> — collision constatée à la relecture ; décisions à déclencheur `date` et actions en échec jamais
> mises en cache ; planification reconnue par l'interpréteur complétée de sa `phrase_originale`
> (§16.4) ; rechargement à chaud par surveillance du dossier `data/ia/` (§12). **Démarrage
> subordonné à HA prêt** (§17, demande utilisateur : « l'ia comme les autres ne peut pas démarrer
> sans ha ready »). **Plus de réinterprétation au déclenchement** (§10, décision utilisateur, en
> miroir de `fonctionnelles-planificateur_specs` v1.11) : `DeployResponder` supprimé, remplacé par
> `ConditionEvaluator` (question vrai/faux sur une condition en texte libre) ; règles
> (`regles_mistral.txt`) : `verbe`/`quoi` obligatoires dans toute action, exemples corrigés.
...
- **Throttling préventif** : espace les requêtes sortantes selon la limite de requêtes/seconde du
  ...
  `IaService`/`ConditionEvaluator` — `DeployResponder` jusqu'à v1.13 — une fois le flux entièrement consommé), jamais avant l'envoi.
...
Mistral (`IaService.runChatRounds()`, et `DeployResponder` jusqu'à v1.13) via le paramètre `prompt_cache_key`
...
à `DeployResponder` (réinterprétation à l'exécution, **supprimée en v1.14**, §10) — sans boucle de relance possible ici
...
## 10. Au déclenchement : plus de réinterprétation, seulement l'évaluation d'une condition (⭐ v1.14)

Jusqu'à v1.13, `planificateur` renvoyait la `phrase_originale` à `ia` à chaque déclenchement
(`planificateur:deploy`, `DeployResponder`) pour une réinterprétation complète par Mistral (macros
déployées, conditions évaluées, aléatoires tirés) — puis mettait la séquence obtenue en cache.
**Décision utilisateur du 24/09/2026** : Mistral ne rend pas toujours la même interprétation d'une
même phrase (« dans 5 minutes » parfois oublié) et le cache figeait ce qui doit varier. La phrase
est désormais interprétée **une seule fois, à la création** ; `planificateur` exécute et recalcule
lui-même la structure à chaque déclenchement (`fonctionnelles-planificateur_specs` §6).

`ia` n'intervient plus au déclenchement que pour une **condition en texte libre** que
`planificateur` ne sait pas évaluer en code (soleil, comparaison numérique et état simple le sont) :
`planificateur:condition` → `ConditionEvaluator` — Mistral reçoit la condition, l'heure/le jour et
l'éventuelle entité déclenchante, avec les seuls outils de **lecture** (`lister_entites`/
`obtenir_etat` ; `executer_action` n'est pas proposé, et `ToolExecutor` en dry-run ne transmettrait
rien), et répond `{"resultat": true|false|null, "raison": ...}`. Attend le référentiel HA (§17).
Conséquence pour les règles (`regles_mistral.txt`) : `verbe`/`quoi` (et `valeur` si besoin) sont
...
| `planificateur:condition` | planificateur → ia | ⭐ v1.14 — évaluer une condition en texte libre (§10) ; remplace `planificateur:deploy` |
| `planificateur:condition:reply` | ia → planificateur | Verdict corrélé (`result` vrai/faux, ou échec) |
...
- **`InterpreterMetrics`** : compteurs `cacheHits`/`interpreterHits`/`mistralCalls`, instance
  partagée par les points d'entrée d'`IaService` (un seul jeu de compteurs). Diffusés dans le statut `ia` existant (§13).
- **Ordre d'essai** : phrases dites (chat direct via `IaService.handleChat()` — HA — §16.2, et test
  du tableau de bord) : cache → interpréteur déterministe → Mistral. (La réinterprétation d'un
  déclenchement planifié, `DeployResponder`, n'existe plus depuis v1.14 — §10.) Le résultat Mistral est mis en cache après coup, pour bénéficier au
  prochain envoi identique — y compris pour une phrase que le moteur déterministe ne reconnaît pas
  du tout.
- **`DeployResponder`** (réinterprétation à l'exécution) : **supprimé en v1.14** (§10). Avant sa
  suppression, il avait déjà cessé d'utiliser `phraseCache` : il rangeait sa séquence `execution`
  sous `phrase_originale` (la phrase complète dite par l'utilisateur), dans le cache partagé avec
  HA — redire cette phrase exécutait la séquence immédiatement au lieu de recréer la planification.
  `outcomesToExecutionSteps()`, qui ne servait qu'à lui, est supprimée aussi.
- **Jamais mis en cache** (v1.14) : une décision contenant un déclencheur `date` (jour absolu,
  souvent calculé depuis « demain »… — rejouée plus tard, elle recréerait une planification dans le
  passé) ; une action Mistral dont le dernier `executer_action` a échoué côté `planificateur`.
- **`phrase_originale` des planifications de l'interpréteur** (v1.14) : l'interpréteur la produit
  vide ; `IaService.executeOutcomes` la complète avec la phrase réellement dite avant transmission
  (sans quoi `planificateur` réinterprétait une phrase vide à chaque déclenchement).
...
FR→EN en v1.12). `si_alors` reste validé uniquement par test unitaire/relecture de code (le chemin
`DeployResponder` a été supprimé en v1.14) — jamais testé en direct contre un vrai
...
- **Démarrage subordonné au premier `ha:ready`** (v1.14, demande utilisateur, même règle que
  `planificateur`) : tant que le référentiel HA n'est pas chargé dans `HaBridgeClient`, le serveur
  Ollama émulé n'est pas ouvert (HA ne peut pas joindre `ia`), le test et le comparatif du tableau
  de bord répondent « HA pas encore synchronisé », et une réinterprétation (`planificateur:deploy`)
  reçue entre-temps attend le référentiel au lieu d'être traitée sans lui. Sans cette règle, Mistral
  tournait sans catalogue quoi/lieux et la vérification des références (§8.2) laissait tout passer.
  Ouverture une seule fois ; une reconnexion HA ultérieure recharge seulement le cache.
