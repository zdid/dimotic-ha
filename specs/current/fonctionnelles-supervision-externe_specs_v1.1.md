...
**Version 1.1 — 25/09/2026 — Claude**

⭐ **v1.1 (25/09/2026)** — décision utilisateur : la **première version** de cette supervision est
l'application dimotic-ha **SUPERVISION** (`fonctionnelles-supervision_specs_v1.0.md`) — inventaire
des machines/applications par gossip, sélection diffusée, état des sauvegardes en face de chaque
machine (§4 ci-dessous, repris et précisé là-bas : informations demandées à l'application
sauvegarde, Nextcloud + `status.json`). Les **actions** (alertes HA, repli SMS/email, §5) et les
nœuds RPi 1 indépendants (§2) restent la suite, à établir dans un second temps. Le reste de ce
document est inchangé.

*(v1.0 — 16/09/2026 — Claude)*
