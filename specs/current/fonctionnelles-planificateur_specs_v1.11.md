...
> plus jamais réinterprétée par Mistral au déclenchement (résultats variables : « dans 5 minutes »
> parfois oublié) : la structure `action` décidée à la création est exécutée et **recalculée en code**
> à chaque exécution ; cache de résolution (`resolvedCache`) supprimé (il figeait conditions,
> aléatoires et entité déclenchante) ; conditions évaluées en code (soleil, numérique, état simple),
> sinon seule question restante posée à `ia` (vrai/faux) ; **plus de repli vers l'agent de
> conversation de HA** (§8 — cet agent est `ia` lui-même) ; validation à la réception (§4) ;
> corrections issues de la revue de code (§3.1, §3.1bis, §3.2, §4, §5.1).
...
système depuis v1.11 — **exécute à chaque déclenchement la structure décidée une fois à la
création, recalculée en code** (entités, conditions, attentes, macros) : la phrase n'est jamais
réinterprétée (§6 ; jusqu'à v1.10, `ia` réinterprétait la phrase à chaque déclenchement).
...
| `execution` | Séquence plate produite par `ia` (ex. interpréteur déterministe), exécutée immédiatement (§6) — jamais stockée |
...
| `window` | "de 18h à 22h" | Récurrent : action à `from` (jours `days`/`except_days` respectés, v1.11), action **inverse** à `to` |
| `duration` | "pendant 10 minutes" | Ponctuel : action **tout de suite**, action inverse après la durée (v1.11) |
| `sun` | "au coucher du soleil", "1h après le lever" | Récurrent, heure recalculée chaque jour (§3.1bis) |

**⭐ v1.11 — fin de plage / de durée** : l'« action inverse » est dérivée en code de la structure
`action` (`execution.ts::inverseOf`) — chaque commande à verbe inversible (allumer↔éteindre,
ouvrir↔fermer, activer↔désactiver), dans l'ordre inverse ; conditions, attentes et macros ignorées ;
rien si aucune commande n'a d'inverse évident. Programmée en mémoire (`CommandHandler.endTimers`) :
une fin en attente est perdue si le service redémarre pendant la plage/durée. Avant v1.11 : `window`
ignorait `to` et les jours, et `duration` attendait la durée PUIS lançait l'action, sans fin.
...
**⭐ v1.11 — correctif démarrage** : la position GPS était demandée sans être attendue — le premier
calcul au démarrage renvoyait toujours « non calculable » et la planification `sun` n'était jamais
programmée (aucun nouvel essai). Désormais `PlanificateurService` attend la position
(`SunTimesSource.ensurePosition()`, après le premier `ha:ready`) avant de programmer, et
`SchedulerRuntime` réessaie toutes les 10 minutes une planification `sun` non calculable (au
démarrage comme au réarmement d'une occurrence). Le même calcul daté sert aux **conditions** sur le
soleil (§6).
...
du déclenchement est ciblée **en code** (v1.11 — voir plus bas) ; seule une condition en texte libre
transmet encore `triggered_entity_id` à `ia` (§6).

**⭐ v1.11 — vrai passage d'état uniquement** : le socle relaie tout `state_changed` de HA, y compris
un simple changement d'attribut (luminosité…) à état égal. `StateWatcher` garde le dernier état connu
de chaque entité (amorcé depuis le référentiel au démarrage) et ne déclenche que sur un **passage** à
`to_state` — sinon une minuterie « quand X s'allume » repartait de zéro à chaque variation.
**Ciblage implicite (v1.11)** : une commande **sans lieu** d'une règle `state_change` vise l'entité
déclenchante si elle fait partie du `quoi` visé (« éteins-la ») — en code, plus via Mistral.
...
**⭐ v1.11 — Validation à la réception** (`CommandHandler.checkPlanification`) : toute planification ou
macro reçue de `ia` est validée AVANT d'être acceptée — schéma de stockage, **chaque commande porte
un `verbe` connu du moteur et un `quoi`** (plus de repli possible, §8), déclencheur calculable. Refus
= réponse d'erreur explicite, rien en mémoire ni sur disque. Avant v1.11, une entrée invalide était
acceptée (« enregistrée »), son écriture échouait, et elle restait en mémoire en faisant échouer
toutes les écritures suivantes jusqu'au redémarrage. Même garde-fou pour `modifier`, qui ne peut
plus changer le `name` (clé de stockage). **`activer`/`modifier` repartent d'une échéance
recalculée** (`next_fire_at`/`missed` effacés — avant, l'ancienne échéance faisait passer une
planification réactivée pour « en retard » : marquée manquée, terminée sans s'exécuter, ou
exécutée immédiatement). **Recréer une planification sous un nom existant désarme d'abord
l'ancienne.** Une commande structurée reçue directement (`macro_ref` — macro dite, `sequence`,
`condition`, `action`) est exécutée immédiatement (avant v1.11 : refusée, « type non pris en
charge »).
...
- **Triggers `state_change`** : `pending` (§2) trace les entités pour lesquelles une exécution était
  ...
  exécution, nouvelle attente complète) plutôt que reprise sur le temps restant — voir §12.
  **⭐ v1.11** : cette reprise attend le premier `ha:ready` (avant : relancée au démarrage, référentiel
  vide → commande ignorée). L'entité finit toujours par recevoir l'action attendue, mais
  potentiellement plus tard qu'en l'absence de redémarrage.

## 6. Exécution d'une planification — structure décidée à la création, recalculée en code (⭐ v1.11)

**Décision utilisateur du 24/09/2026**, remplace la « boucle de déploiement » des versions ≤ 1.10
(réinterprétation complète de `phrase_originale` par Mistral à chaque déclenchement via
`planificateur:deploy`, puis cache de la séquence obtenue — `resolvedCache`, 13/08/2026). Deux
défauts : Mistral ne rend pas toujours la même interprétation d'une même phrase (« dans 5 minutes »
parfois oublié), et le cache, censé l'éviter, figeait ce qui doit varier (condition évaluée une fois
pour toutes, aléatoire tiré une fois, entité déclenchante de la première occurrence réutilisée pour
les suivantes).

**Principe** : la phrase est interprétée **une seule fois**, à la création (`ia`/Mistral ou
interpréteur déterministe) ; la structure obtenue (`action` : action, attente, condition, séquence,
`macro_ref`) est stockée et vérifiable (YAML sur le tableau de bord). À **chaque** exécution —
minuteur, changement d'état, macro dite, commande directe — `ExecutionEngine.run()` la parcourt et
recalcule tout en code :

| Nœud | Traitement à chaque exécution |
|------|------------------------------|
| déclencheur (`trigger`) | Calculé par le code (§3.1), jamais par Mistral — inchangé |
| `action` (verbe/quoi/lieux/valeur) | Résolue contre le référentiel HA courant (§7) → service HA ; rien de mémorisé |
| `wait` | Durée fixe, ou nouveau tirage dans `seconds_min`/`seconds_max` à chaque fois |
| `macro_ref` | Définition **courante** de la macro, même traitement ; boucle détectée (échec tracé) |
| `condition` | En code si possible (`conditions.ts`, ci-dessous), sinon question vrai/faux à `ia` |
| `sequence` | Étapes dans l'ordre |

**Conditions** (`if` : texte libre, ou forme structurée `regles_mistral.txt` §2.3 —
`{phrase, quoi, lieux, signe, valeur}`, acceptée par le schéma depuis v1.11 : avant, elle faisait
échouer l'écriture du fichier) :
- **soleil** (« soleil couché/levé », « nuit/jour ») : même calcul daté que le déclencheur `sun`
  (§3.1bis — lever/coucher du jour, position GPS de HA) ;
- **numérique** (`signe` + `valeur` numérique) : état des entités résolues par `quoi`/`lieux`
  (moyenne si plusieurs) ;
- **état simple** (`valeur` allumé/éteint/ouvert/fermé/détecté/…, `=`/`!=`) : au moins une entité
  dans l'état voulu ;
- **sinon** : `planificateur:condition` → `ia` (`ConditionEvaluator`, outils de lecture seulement,
  réponse `{resultat: true|false|null}`) — **seul appel restant à `ia` au déclenchement**. Condition
  non évaluable → branche non exécutée, échec tracé.

**Bilan** : chaque exécution renvoie le nombre de commandes et les échecs (commande non résolue,
condition non évaluée, macro inconnue…) — un échec positionne `anomalie` sur la planification,
effacée à la prochaine exécution sans échec (`missed` effacé à tout déclenchement effectué).

Une séquence plate reçue directement de `ia` (type `execution`, ex. « allume le salon. attendre 3
heures. éteins le salon. » reconnu par l'interpréteur) passe par le même moteur.
...
*(Jusqu'à v1.10 — réinterprétation à l'exécution supprimée, §6 :)* le contexte envoyé à `ia` (`entities_snapshot`) incluait
...
Si le verbe n'est pas connu de cette table (ambigu, nouveau, non encore prévu) : refusé dès la
...
`HaBridgeClient.sendCommand()`.

**⭐ v1.11 — plus aucun repli** : jusqu'à v1.10, un verbe non couvert par §7 retombait sur l'agent
de conversation natif de HA (`processConversation(order)`). Or cet agent **est `ia`** (émulation
Ollama) : le repli pouvait boucler planificateur → HA → `ia` → planificateur. Une commande non
résolue (verbe inconnu, aucune entité, référentiel indisponible) n'envoie **rien** à HA et est tracée
en échec ; la validation à la réception (§4) refuse dès la création une commande sans `verbe`/`quoi`
ou à verbe inconnu.

**⭐ v1.6 — Deux journaux distincts sur le tableau de bord** :
...
- **Détail des commandes** (`planificateur:ha-commands:list`, 20 dernières) : pour chaque commande
  exécutée ou condition évaluée — issue (`resolved` service HA appelé / `unresolved` non exécutée,
  rien envoyé / `condition` évaluée, avec résultat et détail : valeurs lues, heures du soleil,
  réponse de `ia`), succès/échec, entité à l'origine (`state_change`), prochaine exécution
  (`next_fire_at`) pour un déclencheur temporel récurrent.

## 9. Communication interne (EventBus)

Voir `fonctionnelles-ia_specs` §11 pour la table complète des événements (`ia:command`/`:reply`,
`ia:tool:execute`/`:reply`, et depuis v1.11 `planificateur:condition`/`:reply` — remplace
`planificateur:deploy`/`:reply`) — même helper de corrélation local des
...
délai d'attente de la réponse de `ia` à une évaluation de condition (`deployTimeoutMs`, nom conservé
pour ne pas invalider les fichiers existants), et la fenêtre de
...
- La table verbe→service (§7) ne couvre que les verbes déjà connus des règles domotiques — tout
  ...
  résolution) ; depuis v1.11 une commande à verbe inconnu est **refusée à la création** (§4).
- **Exécution fidèle à la compréhension initiale** (v1.11) : une planification fait exactement ce
  qui a été structuré à sa création — une erreur d'interprétation se voit dans la réponse et le YAML
  et se corrige par « Modifier » ; elle n'est plus « rattrapée » (ni aggravée) au déclenchement.
- Fin de plage `window` / de `duration` programmée en mémoire : perdue si le service redémarre
  pendant la plage (§3.1).
- **Reprise après coupure imprécise pour `state_change`** (§5.1) : contrairement à un trigger
  ...
  d'attente exact restant (l'attente en cours n'est pas persistée). L'entité redéclenche à neuf
