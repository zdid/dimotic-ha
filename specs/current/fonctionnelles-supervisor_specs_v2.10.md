...
*Version 2.10 - 24 Septembre 2026 — §8 réécrit (cycle de vie des applications) : le core ne redémarre
JAMAIS pour une application ; un seul chemin « activer » et un seul « désactiver » ; application
nouvelle = désactivée (y compris installation neuve) ; racine externe `data/applications/` ; nettoyage
MQTT par le core à toute fin de process ; état `restarting`/`crashed` visible. Tout validé en réel avec
l'application de test `testcycle` (§8.7).*
*Historique — Version 2.9 - 19 Septembre 2026 — §15.1 : références mortes corrigées (`inter-app-communication_specs`
...
⭐ **Réécrit le 24/09/2026** (v2.10). Décisions prises avec l'utilisateur le 24/09/2026 :

### 8.1 Principe : le core ne redémarre jamais pour une application

Le core charge l'application, elle annonce ce qu'elle fournit (menu, événements Socket.io, schéma de
config, bridges MQTT), chacun fait sa part — ajout, activation, désactivation, retrait, tout se fait
**à chaud**. Toutes les applications métier sont en process séparé (`runsAsSeparateProcess: true`) ;
l'ancien redémarrage complet de 15 s (`RestartManager.scheduleRestart`) n'est plus utilisé pour les
applications (il reste pour les changements du socle lui-même, ex. `ha.ws`).

### 8.2 Un seul chemin « activer », un seul chemin « désactiver »

`AppService.registerApp()` (chargement du module, entrée dans la liste des modules, schéma de config
Zod `{moduleId}ConfigSchema`, événements Socket.io, câblage du process séparé) est le MÊME au démarrage
du core et à l'activation à chaud — avant le 24/09, l'activation à chaud oubliait le schéma et la
section de config. `activateApp()` y ajoute section de config, démarrage et annonce (menu, formulaire
Paramètres Techniques, gossip). `deactivateApp()` défait tout : arrêt du process (même en attente de
relance), retrait de la liste des modules, et `app:unregistered` pour que **SocketBridge** libère menu,
métadonnées UI, états persistants et écouteurs, et que **IntegrationBridge** désenregistre les bridges
MQTT de l'application. Les handlers client→serveur d'une application activée à chaud sont aussi posés
sur les onglets DÉJÀ ouverts (avant : seulement à la connexion).

### 8.3 Découverte : deux racines, application nouvelle = désactivée

- **Racines** (`application/appRoots.ts`, seul résolveur « appId → dossier ») : `applications/`
  (interne, image) et `data/applications/` (externe, sur le volume `data/`, survit aux mises à jour
  d'image — pour tester une application avant de l'intégrer). L'externe **masque** l'interne de même
  nom. Le core crée le lien relatif `data/applications/core → ../../applications/core` : les
  applications importent le core par chemin relatif (`../../../core/dist/exports`), cassé sinon
  depuis la racine externe (vérifié en réel). Une application externe embarque ses propres
  `node_modules` et tourne avec le core de l'image en place. Code dans `data/applications/<app>/`,
  données dans `data/<app>/`. Noms réservés : `core`, `applications`.
- **Rapprochement disque/config** (`ApplicationManager.reconcile()`, au démarrage et à chaque ouverture
  de Gestion des applications) : `knownApps` (data/core/config.yaml) mémorise les applications déjà
  vues. Une application **jamais vue arrive DÉSACTIVÉE** (ajoutée à `disabledApps`, repère
  « Nouvelle »). Sur une **installation neuve** (fichier de config créé par ce process), toutes les
  applications sont désactivées ; sur une installation existante sans `knownApps`, l'état actuel est
  conservé. Une application **disparue du disque** est arrêtée et oubliée — revenue, elle est de
  nouveau « nouvelle ». Motif : une mise à jour d'image ou une machine neuve ne démarre jamais rien
  (ex. rfxcom sans dongle) ni ne publie d'entités HA sans décision explicite.
- Le dossier d'une application peut changer entre deux activations (externe ajoutée/retirée) :
  `ProcessSupervisor.register()` met à jour le dossier tant que le process est arrêté, la route
  statique `/applications/:appId` sert les fichiers de la version **chargée** (`servedAppDir`), et le
  navigateur oublie la page gardée en cache d'une application retirée de la liste des modules.
...
⭐ **v2.10** : état `restarting` pendant l'attente de relance, distinct de `crashed` (terminal) ;
états relayés à l'interface (`app:process:state`) et visibles dans Gestion des applications (« Plantée
— relance en cours », « Plantée — arrêtée » + bouton **Relancer** qui remet le compteur à zéro).
Désactiver pendant une attente de relance annule la relance (bug corrigé : `stop()` sortait avant
...
### 8.5 Nettoyage MQTT par le core, disponibilité dans HA (v2.10)

Les bridges MQTT des applications d'intégration sont tenus par le core (`IntegrationBridge`). Le core
les désenregistre lui-même (statut `offline` publié) à **toute** fin de process — crash, SIGKILL,
désactivation — sans compter sur le `stop()` propre de l'application. La découverte HA déclare
désormais `availability_topic = <module>/<bridge>/status` (`online`/`offline`) : une application
arrêtée ou plantée rend ses entités **indisponibles** au lieu de figer leur dernière valeur (avant le
24/09, aucune intégration dimotic ne déclarait sa disponibilité).

### 8.6 Application retirée

Dossier supprimé pendant que l'application tourne : à la relecture suivante (ouverture de Gestion des
applications), elle est arrêtée proprement et disparaît. Supprimer les fichiers d'une application qui
tourne rend sa page non servie jusqu'à sa désactivation (la version chargée n'existe plus — voulu :
jamais de mélange de deux versions).

### 8.7 Application de test `testcycle`

`applications/testcycle/` (intégration, process séparé) touche à tout ce que le core gère : MQTT (2
capteurs HA), lecture HA par WebSocket, 3 paramètres techniques dont un **mode de panne** (aucun, plante
au démarrage, plante après 30 s, ignore SIGTERM), 3 données propres sur sa page. Embarquée dans l'image
Docker, désactivée par défaut. Scénarios validés en réel le 24/09/2026 : ajout, activation/désactivation
à chaud, onglet déjà ouvert, `unavailable` dans HA, les 3 modes de panne (backoff 1/2/4/8/16 s,
`crashed`, Relancer, SIGKILL de secours), désactivation pendant l'attente de relance, retrait, racine
externe qui remplace l'interne et retour.
