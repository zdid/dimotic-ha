...
> **v6.2 (24/09/2026)** — **Volets : dernière position republiée vers HA au démarrage** (§9.2bis,
> demande utilisateur), seulement si elle est réellement connue (fichier des derniers états) —
> jamais la position par défaut (100 %), jamais de commande matérielle.

> **v6.1 (24/09/2026)** — **Derniers états sortis de la configuration** (§10.4, décision
> utilisateur : « perdre les derniers états n'est pas grave, mais la config à refaire ça va trop
> loin ») : `lastValue`/`lastSeen`/`lastOn`/`lastLevel`/`lastPosition`/`lastAnyValueChangeAt` vont
> dans `rfxcom-derniers-etats.json` (écriture groupée, au plus une toutes les 30 s) — le fichier de
> config n'est plus réécrit à chaque trame RF (mesuré sur stfort : ~3 600 réécritures/jour + `.bak`).
> Migration automatique au premier démarrage. Reconnexion à chaud du port série réparée (§8.5 : la
> config n'était jamais relue dans le process séparé). Changer le type d'un récepteur = suppression
> + recréation côté HA (§12). Boucle de reconnexion relancée à l'arrêt corrigée.
...
4. Ne touche **pas** `bridgeInstance` (pas de ré-enregistrement du bridge) et ne republie **pas**
   la découverte — seule la connexion série est renouvelée. Un changement de `bridgeInstance` est
   journalisé et ne s'applique qu'au prochain démarrage de l'application (v6.1).

**⭐ v6.1 — correctif** : RFXCOM tourne dans un process séparé qui garde en mémoire la config lue à
son démarrage ; l'étape 2 comparait donc l'ancienne config à elle-même et ne reconnectait jamais.
La config est désormais relue sur disque (`configProvider.reload()`) avant la comparaison, comme
le font déjà rpigpio/teleinfo/sauvegarde.
...
**⭐ v6.2 — volets** : un récepteur `cover` dont la dernière position est connue (`lastPosition`,
fichier des derniers états §10.4) est republié lui aussi au démarrage/reconnexion (état + position),
de façon passive. Position inconnue → rien de publié (le défaut interne de 100 % serait une valeur
inventée) ; le volet reste « inconnu » dans HA jusqu'à la prochaine commande.
...
Mis à jour dans `publishDeviceState()`, persisté avec `lastValue` (depuis v6.1 dans le fichier des
derniers états, écriture groupée — §10.4),
...
### 10.4 Derniers états — fichier séparé (⭐ nouveau v6.1)

**Décision utilisateur du 24/09/2026** : la configuration (devices, récepteurs, scènes, appairages)
ne contient plus aucun « dernier état ». Ces valeurs vivent dans
`data/rfxcom/rfxcom-derniers-etats.json` (`LastStatesStore`) :

| Clé | Contenu |
|-----|---------|
| `devices.<uniqueId>` | `lastValue`, `lastSeen` |
| `receivers.<receiverId>` | `lastOn`, `lastLevel`, `lastPosition` |
| `lastAnyValueChangeAt` | Horodatage global (§9.2ter) |

- **Écriture groupée** : au plus une écriture toutes les 30 s pendant la réception (la photo est
  prise au moment de l'écriture), aucune quand rien n'arrive ; écriture immédiate à l'arrêt propre
  du service. Atomique (tmp → rename), **sans `.bak`** : un fichier perdu ou illisible = on repart
  sans derniers états (entités « inconnues » dans HA jusqu'à la prochaine trame), la config n'est
  jamais touchée. Au pire, une coupure brutale perd les 30 dernières secondes d'états.
- **Config** : réécrite seulement quand elle change réellement — paramétrage depuis l'écran, ou
  nouvelle adresse de commande apprise (`commandDeviceId`, qui reste dans la config : c'est une
  identité, pas un état). Avant v6.1 : réécriture complète (+ `.bak`) à chaque trame RF d'un device
  paramétré — mesuré sur stfort, ~3 600 fois/jour, avec le risque qu'une écriture interrompue abîme
  la config elle-même.
- **Chargement** : les derniers états du fichier dédié sont appliqués avant la construction des
  récepteurs (qui en tirent leur état initial, §9.2bis). **Migration** : une config de l'ancien
  format qui contient encore des derniers états est migrée au premier démarrage (états écrits dans
  le fichier dédié, puis config réécrite sans eux ; l'ancienne version reste dans le `.bak`).
