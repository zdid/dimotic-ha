...
**Document parent :** [fonctionnelles-nommage_specs_v1.7.md](./fonctionnelles-nommage_specs_v1.7.md)

> **v1.7** : `NommageMqttIntegrationService.connectSource()` calcule désormais l'identifiant de
> connexion MQTT via `computeBridgeInstance(source.mqtt.clientId, process.env.DIMOTIC_MACHINE_ID)`
> (import `core/dist/exports`, même fonction que `bridgeInstance` sur arexx/evoo7/rfxcom/rpigpio)
> au lieu d'utiliser `source.mqtt.clientId` tel quel — voir §4.2 (exemple de code mis à jour) et
> `fonctionnelles-nommage_specs` v1.7 pour le contexte complet (`sources[].mqtt.host` corrigé en
> conditions réelles au passage, sans changement de code — juste un `hint` d'avertissement côté UI).
...
|---------|------|--------|-------------|
| **1.7** | 08/09/2026 | Claude | **`connectSource()` calcule désormais l'identifiant de connexion MQTT** (§4.2, exemple de code) via `computeBridgeInstance(source.mqtt.clientId, DIMOTIC_MACHINE_ID)` au lieu de `source.mqtt.clientId` tel quel — même mécanisme que `bridgeInstance` (arexx/evoo7/rfxcom/rpigpio). Voir `fonctionnelles-nommage_specs` v1.7 pour le contexte complet. Ancienne version v1.6 archivée. |
