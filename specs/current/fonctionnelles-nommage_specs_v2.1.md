...
> **v2.1 (24/09/2026)** — **⚠️ Plus de « sources » MQTT : NOMMAGE écoute via la connexion MQTT du
> socle** (décision utilisateur : les sources avaient été prévues pour lire d'autres brokers, ce
> qui n'a jamais servi). **Remplace** tout ce que ce document décrit des sources multiples
> (§3.1 « Sources multiples », §4.3 `sources[]`, la configuration MQTT par source, la reconnexion
> par source, le statut « par connexion ») :
> - **Configuration** : seulement `prefixes: [{prefix}]` (ex. `homeassist`), plus `ha`,
>   `language`, `logging` inchangés. Abonnement, via `integration:nommage:passthrough:subscribe`,
>   à `<prefix>/+/+/config`, `<prefix>/+/+/+/config` et `<prefix>/+/+/+/+/config` (rpigpio) ;
>   messages reçus par `integration:nommage:passthrough:message` ; le socle se réabonne seul à
>   chaque reconnexion. **Migration automatique** : `sources[].mqtt.topicPrefix` → `prefixes`.
> - **Retrait à la source** (message retenu vide, ex. appareil supprimé de zigbee2mqtt) :
>   propagé à HA (config vide retenue sur le topic `homeassistant/…` publié, topic d'attributs
>   effacé). Avant : relayé comme une découverte invalide `{"raw":""}`, entité fantôme.
> - **Taxonomies** : une par topic (plus de doublons ni de croissance sans limite) ; envoi à
>   l'écran au plus une fois par seconde.
> - **Retour de HA** (`ha:online`) : les dernières découvertes relayées sont rejouées directement
>   (au lieu de couper/rouvrir toutes les connexions).
> - **Config relue sur disque** (`configProvider.reload()`) à l'enregistrement — process séparé.
> - Écran : « Préfixes écoutés (MQTT du socle) » au lieu de l'état de chaque source.
> - Limite connue : un préfixe retiré reste abonné côté socle jusqu'au redémarrage de
>   l'application (ses messages sont ignorés). Site de noisy : son Mosquitto n'est pas le broker du
>   socle — ne pas y déployer cette version avant le retour au standard (matériel Pi4).
