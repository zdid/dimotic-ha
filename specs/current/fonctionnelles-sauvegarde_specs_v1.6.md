...
dans `status.json` (§5ter) ; nouvel échange **`sauvegarde:supervision:status`** (§5quater) qui lit
ces informations par SSH sur chaque machine couverte. **Trou constaté** : une machine restaurée n'est
plus sauvegardée (script, cron et secret sont hors de `/docker`, non restaurés) — ha2 depuis le
24/09/2026 ; il faut « 📤 Pousser » après une restauration (§6ter, à intégrer à l'assistant).


⭐ **v1.5 (24/09/2026)** — **premier remplacement réel réussi** : ha2 (Raspberry Pi 4) remplacé par un
Orange Pi 4 Pro (autre matériel) restauré depuis Nextcloud, IP 192.168.1.51 reprise par réservation
DHCP, HA/mosquitto/zigbee2mqtt/dimotic-ha (identité ha2) opérationnels (§6ter). Conséquence : les
images Docker sont désormais téléchargées **dès l'étape 1** (§6ter), une machine neuve n'en ayant
aucune — l'étape 2 les téléchargeait (HA > 1 Go), ce qui allongeait la coupure.

---

**Historique — version 1.4 — 23/09/2026 — Claude**
...
- ⭐ **v1.6 — marqueurs de dernière réussite** : après chaque envoi accepté par Nextcloud, le script
  écrit la date ISO dans `/dimotic-backup/last-<dossier>-<cadence>` (ex. `last-docker-hebdomadaire`)
  — `status.json` ne décrit que la dernière exécution, la date du dernier envoi hebdomadaire s'y
  perdait. `status.json` porte aussi `cadencesEchec` (cadences dont l'envoi a échoué pendant cette
  exécution — un hebdomadaire raté passait inaperçu si le journalier avait réussi).

### 5quater. État des sauvegardes pour la supervision (⭐ v1.6)

Échange corrélé (EventBus, `techniques-socle-ha-mqtt_specs` §9bis) `sauvegarde:supervision:status`
→ `sauvegarde:supervision:status:reply`. Pour chaque machine de `targets[]`, en parallèle, une
lecture SSH (clé unique de dimotic-ha) : présence du script et de son cron, `status.json`, marqueurs
...
- **Étape 1 — aucun redémarrage** : prérequis (curl/tar par apt, Docker par `get.docker.com` — même
  ...
  éléments cochés), mise en place de la version restaurée, puis ⭐ **(24/09/2026) `docker compose
  pull`** de ses images — rien n'est démarré, et le téléchargement se fait pendant que la source
  tourne encore ; échec non bloquant (réessayé par l'étape 2) ; nettoyage du répertoire de travail.
...
**Remplacement réel (24/09/2026)** : étape 2 lancée alors que l'orangepi4pro était encore en .147,
puis bascule DHCP vers .51 et redémarrage — Docker relance tous les conteneurs (`restart:
unless-stopped`). HA, mosquitto, zigbee2mqtt (dongle ConBee II déplacé) et dimotic-ha (identité
`ha2_811876`) opérationnels. Constats : zigbee2mqtt marqué « ÉCHEC du démarrage » tant que le dongle
n'était pas branché (à traiter : état « périphérique absent » + « Réessayer » plutôt qu'un échec
définitif) ; images absentes sur la machine neuve → téléchargées à l'étape 2 (d'où le `pull` en
étape 1) ; l'étape 2 n'a pas pu être lancée directement en .51 (cause non établie).
**Pas encore éprouvés** : installation de Docker, relocalisation `/docker-temp`, cas « place
insuffisante », `pull` en étape 1 sur une vraie machine.
