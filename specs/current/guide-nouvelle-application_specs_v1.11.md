...
**Statut :** Document pratique pour les développeurs — **⚠️ OBSOLÈTE, voir avertissement ci-dessous**  
**Public cible :** Développeurs créant une nouvelle application sans modifier le socle  

> **⚠️ OBSOLÈTE (08/09/2026) — À METTRE À JOUR AVANT UTILISATION.** Ce guide date du 04/08/2026 et
> ne couvre AUCUN des changements d'architecture devenus standard depuis (migration mi-août à
> aujourd'hui) — le suivre tel quel produirait une application construite à l'ancienne, à l'encontre
> de la pratique actuelle. Contenu manquant, au minimum :
> - **Process séparé** (`runsAsSeparateProcess`, `ProcessSupervisor`) — toutes les applications
>   tournent désormais chacune dans son propre process (migration terminée le 24/08/2026).
> - **`HaBridgeClient`** — remplace l'accès direct à `HaStructureRegistry`/`HaWsClient`, obligatoire
>   pour toute application en process séparé ayant besoin de Home Assistant.
> - **`bridgedEvents` / `computeBridgeInstance()` / `DIMOTIC_MACHINE_ID`** — pont d'événements
>   core↔app en process séparé, et convention pour tout identifiant devant rester unique par machine.
> - **`core.disabledApps`** comme seul véritable interrupteur d'activation — le champ `enabled` de
>   chaque schéma de config d'application n'est lu nulle part dans le code (découvert le 06/09/2026).
> - **Cibles multi-machines** (`targets[]`/`haStackTargets`, `TargetCards.js`, socle SSH/SCP partagé
>   `core/infrastructure/remote/`) — pattern standard pour toute application déployant vers du
>   matériel distant.
> - **Registre gossip** (`TargetGossipService`/`AppGossipService`) pour la visibilité inter-machines.
>
> Avant de créer une nouvelle application en s'appuyant sur ce guide, relire au minimum
> `fonctionnelles-supervisor_specs` (dernière version) pour l'architecture process séparé actuelle.

> **NOUVEAU v1.11 (18/09/2026) :** Convention pour une application **non-Docker** (systemd ou cron)
> déployée sous `/dimotic-ha-addons/<nom>/` — décidée en concevant la restauration de
> `fonctionnelles-sauvegarde_specs` (l'unité de sauvegarde y est le répertoire de déploiement entier,
> voir son §4 ; ce dont la restauration a besoin pour redémarrer le bon service en dépend
> directement) : le fichier d'activation vit **dans le répertoire de déploiement lui-même**, pas
> écrit directement dans `/etc/systemd/system/` ou `/etc/cron.d/` comme c'était le cas jusqu'ici
> (voir `teleinfo/DeployService.ts`, à corriger en conséquence) :
> - **Service systemd** : `<REMOTE_DIR>/<nom>.service` — le déploiement l'installe par un **lien
>   symbolique** (`ln -sf <REMOTE_DIR>/<nom>.service /etc/systemd/system/<nom>.service`), jamais une
>   copie séparée à garder synchronisée.
> - **Tâche cron** : `<REMOTE_DIR>/<nom>.cron`, au format crontab(5) **système** (avec la colonne
>   utilisateur, ex. `* * * * * root /chemin/script` — différent du format `crontab -e` sans cette
>   colonne). Même principe de lien symbolique vers `/etc/cron.d/<nom>` — ⚠️ le nom du **lien**
>   installé ne doit contenir aucun point : Debian ignore silencieusement les fichiers pointés dans
>   `/etc/cron.d/` sinon. Le fichier source dans le répertoire peut, lui, s'appeler `<nom>.cron` sans
>   problème (seul le nom du lien compte pour cron).
> - **But** : une seule source de vérité par application, sauvegardée avec le reste (aucune donnée
>   séparée à collecter) — à la restauration, il suffit de vérifier la présence de l'un de ces deux
>   fichiers à la racine du répertoire restauré et de refaire le même lien, sans jamais avoir à
>   deviner ou faire saisir un nom de service.
