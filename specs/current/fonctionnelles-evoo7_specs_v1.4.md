...
> **v1.4 (24/09/2026)** : revue de code — **rechargement à chaud réparé** : la config du boîtier
> n'était jamais relue sur disque (process séparé), un changement d'adresse/identifiants n'était
> donc jamais appliqué ; `configProvider.reload()` avant comparaison. `bridgeInstance` n'est plus
> changé à chaud (identité MQTT déjà enregistrée) — journalisé, appliqué au prochain démarrage.
> Activation du thermostat : découvertes des données forcées publiées seulement après un
...
| 1.4 | 24/09/2026 | Claude | Revue de code : config relue sur disque avant la reconnexion à chaud du boîtier (jamais effective dans le process séparé) ; `bridgeInstance` non modifié à chaud (journalisé) ; thermostat : découvertes publiées après enregistrement réussi. v1.3 archivée. |
