...
(désormais implémentée, plus une conception), §8.10 (texte sur l'écran physique). Conception de
départ ("page libre" = plan sans image, cas spécial propagé partout) **abandonnée en cours de route
sur demande explicite de l'utilisateur** au profit d'une image unie générée UNE FOIS à la création
(§4.3) — `filename` reste obligatoire dans le schéma, aucun cas spécial "sans image" nulle part dans
le code. Plusieurs bugs réels préexistants corrigés au passage en testant cette fonctionnalité :
libellés de statut des interrupteurs génériques enfin corrects (§15.3, bugs #2/#3 révisés),
glisser-déposer cassé après un premier aller-retour en mode édition (nouveau bug #18, corrigé),
dédoublonnage des cibles de déploiement Lovelace face aux auto-annonces gossip (nouveau bug #19,
corrigé côté `core`). Voir §18.3 pour le détail complet.*
...
8. [Frontend — Bootstrap et Composants](#8-frontend--bootstrap-et-composants)
    ...
    - 8.11 [Texte libre (v1.7)](#811-texte-libre-08092026)
...
17. [Génération de cartes Plan Home Assistant (Lovelace) — implémentée, v1.7](#17-génération-de-cartes-plan-home-assistant-lovelace--implémentée-et-déployée-en-conditions-réelles)
    - 17.9 [Texte libre et "page libre" sur Lovelace (v1.7)](#179-texte-libre-et-page-libre-sur-la-carte-lovelace-08092026)
...
floorplans:
  <floorplanId>:                  # clé = identifiant du plan, NON assaini (voir §15)
    filename: <nom de fichier image, assaini>   # ⭐ OBLIGATOIRE, voir §4.3
    positions:
      - entity_id: <string>
        x: <number|null>          # normalisé 0-1, null tant que non placé
        y: <number|null>
    texts:                        # ⭐ 08/09/2026, "texte libre" (§8.11) — même position normalisée
      - id: <string>               # 0-1 que les entités, mais AUCUN entity_id : jamais suivi/
        text: <string>             # commandable (trackedEntityIds, §3.2, ne parcourt que
        x: <number>                # `positions`), jamais lié à un état HA.
        y: <number>
        size: small|medium|large   # défaut medium — même 3 paliers sur les 3 rendus (web, écran
        color: <string CSS/hex>    # ESP32, carte Lovelace), valeurs en px différentes par rendu
```

**`filename` reste OBLIGATOIRE** — ⭐ 08/09/2026, correction de conception explicite de
l'utilisateur : un premier essai avait rendu ce champ optionnel (absent = "page libre", un cas
spécial "pas d'image" propagé dans `FloorPlan.ts`/`HaplanService.ts`/`lovelace-generator.ts`/
`generate_esphome_floorplan.py`), abandonné avant livraison — jugé plus fragile qu'utile. Voir §4.3
pour le mécanisme retenu à la place (image unie générée à la création).
...
l'événement Socket.io `haplan:floorplan:positions:update`, qui porte systématiquement la **liste
complète** des positions **et des textes** du plan concerné — jamais une modification
incrémentale. `texts` (⭐ 08/09/2026) partage le **même debounce 5s côté client** que les positions
(`PositionManager`, §8.5) : une seule sauvegarde différée couvre les deux à la fois. Le serveur :
1. Conserve les positions **et textes** précédents en mémoire.
2. Remplace `floorplan.positions`/`floorplan.texts` par les nouvelles listes, sauvegarde. `texts`
   est optionnel côté payload (rétrocompatibilité d'un vieux client) — absent = inchangé, jamais
   vidé silencieusement.
3. **En cas d'échec d'écriture** : restauration en mémoire de l'état précédent (positions et
   textes), `HAPLAN_SAVE_FAILED`, réémission de la liste (le client recolle sur l'état serveur réel).
...
**Sauvegarde forcée à la fermeture de l'onglet** (⭐ 08/09/2026, bug réel corrigé) : jusqu'ici,
`forceSave()` n'était appelé qu'en sortie de mode édition et lors d'un changement de plan
(`cleanup()`) — fermer directement l'onglet/quitter HAPLAN pendant les 5s de debounce, sans passer
par l'un de ces deux chemins, perdait silencieusement la dernière modification (le timer JS étant
détruit avec la page). `window.addEventListener('beforeunload', ...)` (`dashboard-app.ts`) appelle
désormais `FloorPlanContainer.forceSave()` dans ce cas.
...
**"Page libre" — création sans image fournie (⭐ 08/09/2026)** : la route d'upload (§5) accepte
désormais une requête **sans fichier**, seul `floorplanId` requis (case à cocher dédiée côté UI,
§8.11). Dans ce cas, `HaplanService` **génère lui-même** une image PNG (`createBlankBackgroundPng`,
`image-flatten.ts`) — fond **transparent** (alpha=0, même convention que les vraies images de plan
HAPLAN, pensées pour le fond sombre natif de HAPLAN, pas une couleur opaque qui créerait un
rectangle visible dans l'éditeur web), **portrait, ratio hauteur/largeur 4:3** (1080×1440 — un
premier essai en 16:9 jugé trop étroit, "souvent les plans sont pensés pour être affichés sur le
téléphone", retour utilisateur). Cette image est ensuite écrite/référencée **exactement comme une
image uploadée** — `filename` toujours présent (§4.1), **aucun code en aval (web, Lovelace, ESP32)
ne sait qu'il s'agit d'une page libre** : le contain-fit déjà en place partout (`Math.min(widthRatio,
heightRatio)` côté web, `fit_and_pad()` côté ESP32) s'applique de lui-même, avec le ratio 4:3.
...
- **Route** : `POST /api/haplan/floorplans/upload` — `multer` en mémoire, limite **10 Mo**, liste
  blanche de types MIME (`image/png`, `image/jpeg`, `image/webp`). **`image` est optionnel** (⭐
  08/09/2026, "page libre" — voir §4.3) : seul `floorplanId` est requis.
  - Erreur de filtre/multer → **415**.
  - `floorplanId` manquant → **400**.
...
**Texte libre sur l'écran physique (⭐ 08/09/2026)** : chaque texte (§8.11) devient un `label:` LVGL
statique (`generate_esphome_floorplan.py::build_text_widget()`, §16), même transformation de
coordonnées que les icônes. Police par **palier de taille** (small/medium/large, pas par texte —
LVGL référence une police par id) déclarée uniquement pour les paliers réellement utilisés, glyphes
restreints aux caractères réellement présents dans les textes de ce palier à travers **tous** les
plans (accents français compris, calculé dynamiquement) — même principe que la police des valeurs de
...
Élément positionné comme une icône d'entité (§8.6), mais **sans `entity_id`** — un simple contenu
texte affiché à un endroit fixe du plan (ex: un titre de zone, une légende). Bouton dédié
« 🔤 Ajouter un texte » à côté du sélecteur d'entité (`btn-add-text`) : ouvre une invite pour le
contenu, place le texte au centre du plan (`{x:0.5, y:0.5}`), à repositionner ensuite par
glisser-déposer comme n'importe quelle icône.

- **Classe `HAText`** (`models/objects/HAText.ts`) — **volontairement pas une sous-classe de
  `HAObject`** (pas d'état HA, pas de `commandService`) : rendu, glisser-déposer (même mécanique que
  `HAObject`, mode `'center'`, snap 2%) et suppression par glisser sur la poubelle sont réimplémentés
  directement plutôt qu'hérités, pour ne pas alourdir `HAObject` d'un cas qui ne le concerne pas.
- **Édition de contenu** : double-clic sur le texte fait apparaître un `<textarea>` positionné au
  même endroit (pas de conflit avec le glisser-déposer, posé sur l'élément d'origine, temporairement
  masqué) — `Entrée` valide, `Échap` annule, perte de focus valide.
- **3 paliers de taille** (`small`/`medium`/`large`, défaut `medium`) et une **couleur** (défaut
  blanc) — mêmes paliers utilisés sur les 3 rendus (web, écran ESP32 §8.10, carte Lovelace §17),
  valeurs en px différentes par rendu (échelles d'écran différentes). Taille appliquée en CSS (classe
  `haplan-text-{size}`, pas de style inline) pour rester compatible avec `--plan-scale` (§12.2).
- **Persistance** : même debounce 5s que les positions, même sauvegarde forcée en sortie de mode
  édition/changement de plan/fermeture d'onglet (§4.2).
...
`climate.`→thermostat, `input_boolean.`→interrupteur générique (⭐ 08/09/2026, §9.4) ; tout le reste
→ lumière par défaut). Pour les domaines `light` et `switch`, un second niveau de détection
(`SwitchTypeDetector`) peut **reclasser** l'objet en VMC/chauffe-eau/radiateur selon des mots-clés
dans l'`entity_id` ou les attributs — voir limitation §15 (reclassement erroné possible d'une
lumière). **`input_boolean` en est délibérément exclu** (§9.4) : un réglage nommé par exemple
"petit ballon en premier" contient "ballon" sans être un chauffe-eau.

### 9.4 Réglages manuels HA (`input_boolean`, ⭐ 08/09/2026)

Un `input_boolean.*` (helper HA — réglage manuel utilisé par une automation, ex: "petit ballon en
premier") est un domaine à part entière, distinct de `switch`/`light` :
- **Type dédié** dans `UnifiedObjectFactory` (`'input_boolean'`), **hors de la branche `isSwitch`**
  qui déclenche `SwitchTypeDetector` pour `light`/`switch` — un `input_boolean` n'est donc jamais
  reclassé par erreur en VMC/chauffe-eau/radiateur sur la base de mots-clés dans son nom.
- **Rendu** : `EnhancedSwitchObject` (même icône bascule ON/OFF que l'interrupteur générique),
  fenêtre `SwitchWindow` (§10).
- **Domaine dérivé dynamiquement** de l'`entity_id` (`getDomain()`, `entity_id.split('.')[0]`) plutôt
  qu'un littéral `'switch'` en dur — envoie donc bien `input_boolean.turn_on`/`turn_off`, pas
  `switch.turn_on`/`turn_off` sur une entité qui n'est pas un switch. Correction faite en même temps
  sur `EnhancedSwitchObject` en général (bénéficie aussi aux vrais `switch.*`, comportement
  inchangé pour eux puisque leur domaine réel est déjà `switch`).
- **Classement dans le sélecteur d'entités (§6)** : indépendant des pièces HA natives — un
  `input_boolean` créé via l'UI HA (pas via les intégrations MQTT du projet) n'a pas d'attribut
  `attributs_taxonomie` et apparaît donc sous « Non classé / Autre », même si son entité a par
  ailleurs une pièce HA native assignée (deux mécanismes de classement totalement indépendants).
...
| Interrupteur (`EnhancedSwitchObject`/`MinimalLightObject`) | bascule ON/OFF, **glyphe ET libellé désormais mis à jour à chaque changement d'état** (⭐ 08/09/2026, voir §15.3 bugs #2/#3 révisés) | `switch.turn_on`/`turn_off` |
| Réglage manuel (`input_boolean.*`, ⭐ 08/09/2026, §9.4) | même rendu que l'interrupteur générique | `input_boolean.turn_on`/`turn_off` (domaine dérivé dynamiquement, jamais `switch.*` en dur) |
| Volet/Store (`EnhancedCoverObject`/`EnhancedBlindObject`) | fenêtre | `cover.open_cover`/`close_cover`/`stop_cover` |
| VMC/Chauffe-eau/Radiateur | icône dédiée par type — **état lu uniquement à la couleur de l'icône**, aucun libellé ON/OFF (retiré en v1.1, voir §15.3 bug #3) : chauffe-eau bleu éteint/orange allumé, VMC blanc éteint/jaune allumé, radiateur bleu éteint/rouge allumé (+ icône flocon/feu selon l'état, **glyphe désormais bien mis à jour à chaque changement — ⭐ 08/09/2026, même bug que l'interrupteur, corrigé sur le radiateur uniquement : VMC/chauffe-eau ont une icône unique par conception, non concernés**) | `switch.*` |
...
| Texte libre (`HAText`, ⭐ 08/09/2026, §8.11) | texte, pas d'icône — **pas un `HAObject`** | aucune (pas d'entité HA, jamais commandable) |
...
| `SwitchWindow` | Interrupteur générique, `input_boolean` (§9.4) | **Deux boutons explicites Éteindre/Allumer** (⭐ 08/09/2026 — remplace l'ancien bouton bascule unique dont le libellé dépendait d'un état pas toujours fiable, notamment pour des équipements RF ; même patron que `SwitchContextWindow` ci-dessous), fermeture automatique 400ms après action |
...
| 2 | ~~Libellés de statut des interrupteurs génériques et SwitchContextWindow toujours faux~~ **CORRIGÉ le 08/09/2026** | Cause : `SwitchWindow` lisait `getDisplayValue('status')`, un mécanisme dont `EnhancedLightObject.updateState()`/`EnhancedSwitchObject.updateState()` ne renseignent plus rien depuis le passage à "l'affichage simplifié" (code mort mutuel jamais recroisé). Corrigé en **remplaçant le bouton bascule unique par deux boutons explicites** Éteindre/Allumer (§10) plutôt qu'en ressuscitant l'ancien mécanisme — n'a plus besoin de connaître l'état pour choisir le bon libellé, et couvre au passage les équipements à retour d'état peu fiable (RF notamment). `SwitchContextWindow` (VMC/ballon/radiateur) avait déjà toujours les deux boutons, seul son affichage de statut texte restait sur l'ancien mécanisme (non corrigé, mineur — affichage superflu, les boutons fonctionnaient déjà) |
| 3 | ~~Sélecteurs CSS de statut désaccordés du DOM réellement rendu~~ **CORRIGÉ le 08/09/2026 (interrupteur générique) — inchangé pour VMC/chauffe-eau/radiateur** | `EnhancedSwitchObject` (interrupteur générique, `input_boolean` inclus depuis §9.4) : la div de statut était créée sans la classe que `updateDisplay()` recherchait ensuite (`switch-status-display` ajoutée). Bug jumeau trouvé et corrigé dans la foulée : le **glyphe** de l'icône (`fa-toggle-on`/`fa-toggle-off`) n'était mis à jour qu'en couleur, jamais en classe — figé sur celui du premier rendu. Même bug retrouvé et corrigé sur `EnhancedRadiatorObject` (glyphe feu/flocon). **Ne concerne pas `EnhancedWaterHeaterObject`/`EnhancedVMCObject`** : ceux-ci ont volontairement une icône unique par type (pas de glyphe qui change), décision utilisateur de la v1.1, toujours en vigueur — voir §9.3 |
...
| 18 (⭐ 08/09/2026) | ~~Glisser-déposer cassé après un premier aller-retour en mode édition~~ **CORRIGÉ** | `FloorPlan.cleanupTrashDropZone()` (appelée à la sortie du mode édition) appelait `object.render()` au lieu de `object.getElement() \|\| object.render()` — `render()` crée à chaque appel un élément **flambant neuf, jamais inséré dans le DOM** ; le `element.replaceWith(clone)` qui suit est alors un no-op silencieux (spec DOM : nœud sans parent), et la référence interne de l'objet (et d'un texte libre, §8.11) était réassignée à un **clone orphelin**, jamais rattaché à `.floorplan-drag-container`. Sans effet visible immédiatement, mais `enableDrag()` au prochain passage en mode édition cherchait le conteneur depuis cet orphelin et échouait pour **tous** les objets/textes (`Drag container not found after 2 seconds` en console) — plus aucun déplacement ni suppression possible après un premier aller-retour ON/OFF du mode édition. Trouvé en testant le texte libre (§8.11), mais touchait déjà les entités avant cette fonctionnalité |
| 19 (⭐ 08/09/2026, côté `core`, pas HAPLAN) | Dépôt de la carte Lovelace refusé malgré une seule vraie cible HA configurée | `AppService.handleHaplanLovelaceDeploy` (§17.5) comptait `getHaStackTargets().length`, qui inclut les auto-annonces gossip (`origin: 'gossip'`, `TargetGossipService`) — cette même machine s'annonce sous un `machineId` différent à chaque redémarrage, sans jamais nettoyer les anciennes annonces (constaté en réel : 5 entrées pour un seul hôte réel). Corrigé en filtrant sur `origin === 'local'` avant de compter — seules les cibles que l'utilisateur a lui-même configurées sur cette instance comptent pour ce dépôt |
...
└── src/
    ...
    └── presentation/
        ...
        └── haplan/
            ...
            │       ├── HAText.ts                # ⭐ 08/09/2026, texte libre (§8.11) — PAS une sous-classe de HAObject
...
Outil hors runtime (exécuté à la main, voir §8.10) : `applications/haplan/tools/
generate_esphome_floorplan.py` — génère la configuration ESPHome (image de fond ajustée + widgets
LVGL, dont les textes libres depuis §8.11, ⭐ 08/09/2026) pour l'écran mural physique, à partir de la
même config YAML que le reste de l'application.

---

## 17. Génération de cartes Plan Home Assistant (Lovelace) — IMPLÉMENTÉE et déployée en conditions réelles

**Statut (⭐ mis à jour 08/09/2026)** : entièrement **implémentée, testée et déployée en conditions
réelles** sur l'instance HA de l'utilisateur — les §17.1-17.4 ci-dessous restent la conception
d'origine (28/08/2026, toujours valable), les §17.5-17.8 ont été **réécrits pour refléter
l'implémentation réelle**, elle-même affinée par de nombreux retours utilisateur en conditions
réelles entre le 28/08 et le 08/09/2026 (jamais reflétés dans une mise à jour de spec avant cette
version — voir §18.3). Nouveau en 08/09/2026 : **texte libre** (§8.11) et **"page libre"** (§4.3)
couverts sur cette sortie aussi (§17.9, nouvelle).
...
- **Format de sortie** : `lovelace-generator.ts::buildLovelaceDashboardYaml()` construit
  **directement** le YAML attendu par la carte Plan Lovelace de HA (pas de format intermédiaire).
- **Canal d'envoi** : dépôt de fichier par SSH (pas WebSocket, pas MQTT) — voir §17.8, mécanisme
  **implémenté et vérifié en conditions réelles**.
- **Authentification** : sans objet pour ce canal — dépôt de fichier par SSH, pas d'appel à l'API HA.
- **Déclenchement** : bouton dédié « 🏠 Déployer sur HA » (`#btn-deploy-lovelace`, même emplacement
  que le bouton de déploiement écran, §8.9) — action manuelle, pas de génération/envoi automatique à
  chaque modification d'un plan. Verrou simple (`lovelaceDeployInProgress`, même patron que le
  déploiement écran §3.6) — indépendant de ce dernier (cible et mécanisme totalement différents, pas
  de raison de bloquer l'un pendant que l'autre tourne).
- **Durée observée en conditions réelles** : ~2 secondes (génération YAML + copie SSH), bien plus
  rapide que le déploiement écran (§3.6, ~15-65s, compilation ESPHome).
...
### 17.7 Capacités réelles de la carte Plan Lovelace HA — vérifiées en conditions réelles

Contrairement à la v1.5 (capacités déduites de la documentation officielle seule), tout ce qui suit
est **vérifié en direct** contre l'instance HA de l'utilisateur, y compris plusieurs comportements
**non documentés** découverts en testant.

**Le type utilisé est bien `picture-elements`**, avec `state-icon` (icônes), `state-label` (valeurs
de capteur, à côté de l'icône — HAPLAN affiche toujours les deux ensemble, comme sur le plan web) et
`action-button` (texte libre, §17.9 — **pas** `markdown`, qui n'existe pas comme type d'ÉLÉMENT de
`picture-elements`, à ne pas confondre avec la carte `markdown`, un type de CARTE distinct — premier
essai invalide, "Configuration error" affiché à la place de chaque texte).

- **Coordonnées** : `style: {left: X%, top: Y%}` + `transform: translate(-50%,-50%)`, confirmé
  identique à la convention HAPLAN (centre de l'icône).
- **Couleur dynamique par état** : `state_color` natif **insuffisant en pratique** — HAPLAN a besoin
  des couleurs spécifiques par sous-type (VMC/ballon/radiateur, capteurs par type de mesure) qu'HA ne
  peut pas deviner. Nécessite **`card_mod`** (intégration HACS tierce, pas native — prérequis non
  anticipé en v1.5) : perçage de shadow DOM multi-niveaux pour atteindre l'icône réelle
  (`state-badge$: {ha-state-icon$: 'style CSS'}`, la syntaxe `$` traverse une frontière de shadow DOM
  par usage) — couleur forcée par gabarit Jinja (`{% if is_state(...) %}`) plutôt qu'une couleur
  figée, pour retrouver le "bleu éteint / couleur allumé" du plan web.
- **Fenêtre thermostat** : `more-info` natif confirmé suffisant, aucun repli +/- nécessaire.
- **Fond de l'image** : les images de plan HAPLAN sont **transparentes** (pensées pour le fond sombre
  natif de HAPLAN) — invisibles ("blanc sur blanc") sur le fond clair par défaut d'une `ha-card`.
  Résolu en **aplatissant l'image sur un fond sombre uni** (`image-flatten.ts::
  flattenPngOntoDarkBackground`, `#0d0d0d`) **une fois, au dépôt** — l'original local, servant aussi
  à HAPLAN lui-même, n'est jamais modifié.
- **Cache navigateur d'image** : un redéploiement sur la **même** URL `/local/<fichier>` peut rester
  invisible côté navigateur (cache HTTP d'image, indépendant d'un rechargement complet de la page) —
  paramètre `?v=<horodatage>` ajouté à chaque dépôt pour forcer le retéléchargement.
- **Dimensionnement de la carte** : `picture-elements` seule ne dimensionne pas la carte au ratio de
  l'image — nécessite `card_mod` sur `ha-card`/`#root` (le vrai conteneur qui fixe la taille, dans le
  shadow DOM de `hui-picture-elements-card` elle-même, atteignable directement sans perçage) avec un
  `aspect-ratio` **calculé au dépôt** à partir des dimensions réelles de chaque image
  (`image-dimensions.ts`, lecture PNG/JPEG sans dépendance externe) — sans ça, les icônes superposées
  (positionnées en % du conteneur, verrouillé par HA) se décalent dès que le ratio de l'image diffère
  de celui de l'écran.
- **Autres options utiles de `state-icon`/`state-label`** : `entity` (requis), `icon` (surcharge),
  `tap_action`/`hold_action`/`double_tap_action` (`more-info` par défaut au clic — suffisant, aucune
  action custom nécessaire).
...
temps que le YAML), servie nativement par HA au chemin `/local/<fichier>` — **confirmé en réel**.
Choix cohérent avec l'approche par fichier : le rendu ne dépend jamais du serveur web de HAPlan
lui-même, utile si HA est consulté hors du réseau local (appli mobile) sans que HAPlan y soit
forcément joignable.

**Sélection de cible SSH — bug réel corrigé (⭐ 08/09/2026, voir §15.3 bug #19)** : `haStackTargets`
contient aussi les auto-annonces gossip d'une même machine sous des `machineId` différents à chaque
...
- **Texte libre** (§8.11) : élément `action-button` (§17.7 — pas `markdown`), `title` = le texte
  visible, `action` obligatoire côté HA mais neutralisé (`pointer-events: none` empêche tout clic
  réel — service inoffensif choisi par prudence, `persistent_notification.dismiss`).
- **Couleur du texte — bug réel, corrigé après inspection DOM en direct** : ni `color`, ni les
  variables CSS `--primary-color`/`--mdc-theme-primary` (deux hypothèses testées, aucune ne
  fonctionnait) ne suffisaient. Le texte traverse en réalité `hui-service-button-element`
  (implémentation de `action-button`) → shadow → `ha-call-service-button` → shadow →
  `ha-progress-button` → shadow → `ha-button` (composant "Web Awesome", pas Material Design comme
  attendu) → shadow → `<button part="base">`, avec le texte **slotté** (hérite du style de sa
  position dans l'arbre APLATI, pas de son ascendance en lumière — un style posé sur notre propre
  élément ne l'atteint donc jamais). Résolu via **`::part()`** (mécanisme standard pour styliser
  depuis l'extérieur une partie explicitement exposée par un web component, `part="base"`/
  `part="label"` ici), combiné à `card_mod` pour percer les deux frontières intermédiaires qui
  n'exposent rien elles-mêmes : `ha-call-service-button$: {ha-progress-button$: 'ha-button::
  part(label) { color: ... !important; }'}`.
- **"Page libre"** (§4.3) : aucun cas spécial — l'image unie générée à la création est traitée
  exactement comme n'importe quelle image de plan (aplatissement, `aspect-ratio`, tout §17.7
...
| Texte libre (`HAText`, ⭐ v1.7) | Élément de texte positionné comme une icône (§8.11), sans `entity_id` — jamais suivi/commandable |
| "Page libre" (⭐ v1.7) | Plan créé sans image fournie par l'utilisateur — reçoit une image unie générée à la création (§4.3), traitée ensuite exactement comme une image uploadée (aucun cas spécial dans le code) |
