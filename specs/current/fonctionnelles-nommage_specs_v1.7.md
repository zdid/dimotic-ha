...
> **v1.7** : **`sources[].mqtt.host`/`clientId` corrigés** (§4.3), trouvé en travaillant la
> conception "duplication config multi-machines" (voir `fonctionnelles-supervisor_specs`) :
> - **`host`** : `data/nommage/config.yaml` avait `127.0.0.1` (boucle locale) pour la source
>   `ha2.local` — cassait tout dès qu'on sortait du cas "nommage tourne sur la même machine que le
>   broker". Corrigé en `192.168.1.51` (adresse réelle du broker sur `ha2`). Un `hint` d'avertissement
>   permanent a été ajouté sur ce champ (`NOMMAGE_UI_METADATA`) : ne jamais y remettre
>   `127.0.0.1`/`localhost`, même quand le broker tourne sur la même machine que `nommage`.
> - **`clientId`** : n'est plus une valeur devant être unique GLOBALEMENT (§4.3, règle assouplie) —
>   c'est désormais un **PRÉFIXE** seul, même mécanisme que `bridgeInstance` sur arexx/evoo7/rfxcom/
>   rpigpio (`computeBridgeInstance()`, socle `core`) : l'identifiant réellement utilisé pour la
>   connexion MQTT est calculé à la connexion en y ajoutant l'identifiant de la machine
>   (`DIMOTIC_MACHINE_ID`), jamais persisté sous cette forme suffixée. Reste éditable comme avant,
>   mais `data/nommage/config.yaml` peut désormais être dupliqué tel quel sur une autre machine sans
>   risque de collision de `clientId` si `nommage` y était activé en parallèle.
>
