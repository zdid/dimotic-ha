/**
 * Bootstrap autonome d'evoo7 en process séparé — fonctionnelles-supervisor_specs v2.6 §5.2, même
 * patron que applications/arexx/src/standalone.ts. Lancé par ProcessSupervisor
 * (applications/core/src/supervisor/), jamais directement par AppService.startApplicationService()
 * (voir le flag `runsAsSeparateProcess` sur EVOO7_APP).
 *
 ...
 * sa propre connexion au broker EVOO7 dédié, 192.168.1.53, reste inchangée, c'est un usage MQTT
 * totalement indépendant de ce canal EventBus). `createEvoo7ServiceWithConfig()` (domain/index.ts)
 * reste inchangée : c'est ce contrat de factory identique qui permet à ce fichier d'être le seul
 * changement nécessaire pour la migration.
 */
...
import { createEvoo7ServiceWithConfig } from './domain';
...
async function main(): Promise<void> {
  ...
  // permet de lire à la fois core.machineId (juste informatif ici) et sa propre section `evoo7`,
  ...
  const service = createEvoo7ServiceWithConfig(eventBus, logger, configService);

  await service.start();
  logger.info('evoo7:standalone', `evoo7 démarré en process séparé (pid ${process.pid}, machine ${machineId})`);

  let stopping = false;
  const shutdown = async (signal: string): Promise<void> => {
    if (stopping) return;
    stopping = true;
    logger.info('evoo7:standalone', `Signal ${signal} reçu — arrêt`);
    try {
      await service.stop();
    } finally {
      process.exit(0);
...
main().catch((error) => {
  console.error('[evoo7:standalone] Échec du démarrage:', error);
  process.exit(1);
});
