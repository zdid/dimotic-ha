/**
 * Bootstrap autonome de teleinfo en process séparé — fonctionnelles-supervisor_specs v2.6 §5.2,
 * même patron que applications/rpigpio/src/standalone.ts. Lancé par ProcessSupervisor
 * (applications/core/src/supervisor/), jamais directement par AppService.startApplicationService()
 * (voir le flag `runsAsSeparateProcess` sur TELEINFO_APP).
 *
 ...
 * `createTeleinfoServiceWithConfig()` (domain/index.ts) reste inchangée : c'est ce contrat de
...
import { createTeleinfoServiceWithConfig } from './domain';
...
async function main(): Promise<void> {
  ...
  // permet de lire à la fois core.machineId (juste informatif ici) et sa propre section `teleinfo`,
  ...
  const service = createTeleinfoServiceWithConfig(eventBus, logger, configService);

  await service.start();
  logger.info('teleinfo:standalone', `teleinfo démarré en process séparé (pid ${process.pid}, machine ${machineId})`);

  let stopping = false;
  const shutdown = async (signal: string): Promise<void> => {
    if (stopping) return;
    stopping = true;
    logger.info('teleinfo:standalone', `Signal ${signal} reçu — arrêt`);
    try {
      await service.stop();
    } finally {
      process.exit(0);
...
main().catch((error) => {
  console.error('[teleinfo:standalone] Échec du démarrage:', error);
  process.exit(1);
});
