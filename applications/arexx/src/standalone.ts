/**
 * Bootstrap autonome d'arexx en process séparé — fonctionnelles-supervisor_specs v2.6 §5.2, même
 * patron que applications/rpigpio/src/standalone.ts et applications/teleinfo/src/standalone.ts.
 * Lancé par ProcessSupervisor (applications/core/src/supervisor/), jamais directement par
 * AppService.startApplicationService() (voir le flag `runsAsSeparateProcess` sur AREXX_APP).
 *
 ...
 * `createArexxServiceWithConfig()` (domain/index.ts) reste inchangée : c'est ce contrat de factory
...
import { createArexxServiceWithConfig } from './domain';
...
async function main(): Promise<void> {
  ...
  // permet de lire à la fois core.machineId (juste informatif ici) et sa propre section `arexx`,
  ...
  const service = createArexxServiceWithConfig(eventBus, logger, configService);

  await service.start();
  logger.info('arexx:standalone', `arexx démarré en process séparé (pid ${process.pid}, machine ${machineId})`);

  let stopping = false;
  const shutdown = async (signal: string): Promise<void> => {
    if (stopping) return;
    stopping = true;
    logger.info('arexx:standalone', `Signal ${signal} reçu — arrêt`);
    try {
      await service.stop();
    } finally {
      process.exit(0);
...
main().catch((error) => {
  console.error('[arexx:standalone] Échec du démarrage:', error);
  process.exit(1);
});
